...
const useChatStore = defineStore({
  ...
  getters: {
    ...
    getOtherProfile() {
      return this.selectedUser.member.profile
    },
    getOtherMember() {
      return this.selectedUser.member
    },
    isError() {
      return this.error !== ''
    },
  ...
  actions: {
    ...
    async setRoomMessages() {
      ...
      try {
        const { data } = await axios.get('/chat/messages/' + this.room.id, {
          params: { take: 100 }
        })
    ...
    kickMember() {
      this.globalStore.socketKick(this.SelectedUser.id)
    },
    banMember() {
      this.globalStore.socketBan(this.selectedUser.id)
    },
    promoteMember() {
      this.globalStore.socketPromote(this.selectedUser.id)
    },
    ...
    async setDmMessages() {
      this.messages.splice(0)
      try {
        const { data } = await axios.get('/chat/dm/' + this.dmReceiver.id, {
          params: { take: 100 }
        })
        this.messages.push(...data)
      } catch (e) {
    ...
    async addDmMessage(message: any) {
      if (this.isConversationMessage(message)) {
        ...
        const senderInConversations = this.conversations.find(
          (conversation) => conversation.id === message.senderId
        )
