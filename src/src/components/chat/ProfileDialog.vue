  ...
  <v-dialog v-model="globalStore.dialogs.profile" max-width="400">
    <v-card>
      ...
      <v-card-text>
        ...
        <div class="profile-actions">
          <v-btn v-if="chatStore.selectedUserIsNotMe" @click="usersStore.blockUser"
            >Block User</v-btn
          >
          <v-btn v-if="chatStore.selectedUserIsNotMe" @click="usersStore.addFriend"
            >Add Friend</v-btn
          >
          <v-btn v-if="chatStore.selectedUserIsNotMe" @click="chatStore.sendPrivateMessage"
            >Send Message</v-btn
          >
          <v-btn v-if="chatStore.selectedUserIsNotMe" @click="globalStore.inviteToPlay"
            >Invite to Play</v-btn
          >
        </div>
      </v-card-text>
      <v-card-actions>
