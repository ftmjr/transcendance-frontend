<template>
  <div class="flex my-1 align-center text-medium-emphasis">
    ...
    <template v-if="messageStore.conversationWith">
      ...
      <v-chip v-if="blockedStatus !== 'none'" class="ml-2" append-icon="tabler-lock" color="error">
        <template v-if="blockedStatus === 'mutual'"> Mutuellement bloquer </template>
        <template v-else-if="blockedStatus === 'blocked'"> Vous avez bloquer </template>
        <template v-else-if="blockedStatus === 'blockedBy'"> Vous a bloquer </template>
      </v-chip>
      <VSpacer />
