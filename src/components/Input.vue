<template>
  <div class="">
    <label :for="name" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
      {{ label }}</label
    >
    <div class="relative">
      <input
        :value="value"
        :type="(showPassword && 'text') || type"
        :name="name"
        :id="name"
        :maxlength="maxLength"
        :min="min"
        :max="max"
        :placeholder="placeholder"
        :class="[
          (error || typeError) && 'border-red-600 focus:border-red-600',
          classnames ? classnames : styles[type]
        ]"
        @input="handleInput"
        @keydown="handleKeyPress"
        :required="required"
        autocomplete="off"
      />
      <button
        v-if="type === 'password'"
        type="button"
        class="w-[18px] h-[18px] absolute top-1/2 right-2 transform -translate-y-1/2"
        @click="(e) => togglePassword(e)"
      >
        <span v-show="!showPassword">
          <svg
            clip-rule="evenodd"
            fill-rule="evenodd"
            stroke-linejoin="round"
            stroke-miterlimit="2"
            viewBox="0 0 24 24"
            xmlns="http://www.w3.org/2000/svg"
            :class="iconClasses"
          >
            <path
              d="m11.998 5c-4.078 0-7.742 3.093-9.853 6.483-.096.159-.145.338-.145.517s.048.358.144.517c2.112 3.39 5.776 6.483 9.854 6.483 4.143 0 7.796-3.09 9.864-6.493.092-.156.138-.332.138-.507s-.046-.351-.138-.507c-2.068-3.403-5.721-6.493-9.864-6.493zm8.413 7c-1.837 2.878-4.897 5.5-8.413 5.5-3.465 0-6.532-2.632-8.404-5.5 1.871-2.868 4.939-5.5 8.404-5.5 3.518 0 6.579 2.624 8.413 5.5zm-8.411-4c2.208 0 4 1.792 4 4s-1.792 4-4 4-4-1.792-4-4 1.792-4 4-4zm0 1.5c-1.38 0-2.5 1.12-2.5 2.5s1.12 2.5 2.5 2.5 2.5-1.12 2.5-2.5-1.12-2.5-2.5-2.5z"
              fill-rule="nonzero"
            />
          </svg>
        </span>
        <span v-show="showPassword">
          <svg
            clip-rule="evenodd"
            fill-rule="evenodd"
            stroke-linejoin="round"
            stroke-miterlimit="2"
            viewBox="0 0 24 24"
            xmlns="http://www.w3.org/2000/svg"
            :class="iconClasses"
          >
            <path
              d="m17.069 6.546 2.684-2.359c.143-.125.32-.187.497-.187.418 0 .75.34.75.75 0 .207-.086.414-.254.562l-16.5 14.501c-.142.126-.319.187-.496.187-.415 0-.75-.334-.75-.75 0-.207.086-.414.253-.562l2.438-2.143c-1.414-1.132-2.627-2.552-3.547-4.028-.096-.159-.144-.338-.144-.517s.049-.358.145-.517c2.111-3.39 5.775-6.483 9.853-6.483 1.815 0 3.536.593 5.071 1.546zm2.319 1.83c.966.943 1.803 2.014 2.474 3.117.092.156.138.332.138.507s-.046.351-.138.507c-2.068 3.403-5.721 6.493-9.864 6.493-1.297 0-2.553-.313-3.729-.849l1.247-1.096c.795.285 1.626.445 2.482.445 3.516 0 6.576-2.622 8.413-5.5-.595-.932-1.318-1.838-2.145-2.637zm-3.434 3.019c.03.197.046.399.046.605 0 2.208-1.792 4-4 4-.384 0-.756-.054-1.107-.156l1.58-1.389c.895-.171 1.621-.821 1.901-1.671zm-.058-3.818c-1.197-.67-2.512-1.077-3.898-1.077-3.465 0-6.533 2.632-8.404 5.5.853 1.308 1.955 2.567 3.231 3.549l1.728-1.519c-.351-.595-.553-1.289-.553-2.03 0-2.208 1.792-4 4-4 .925 0 1.777.315 2.455.843zm-2.6 2.285c-.378-.23-.822-.362-1.296-.362-1.38 0-2.5 1.12-2.5 2.5 0 .36.076.701.213 1.011z"
              fill-rule="nonzero"
            />
          </svg>
        </span>
      </button>
    </div>
    <span v-if="error" class="text-xs text-red-600">
      {{ errorMessage }}
    </span>
  </div>
</template>
<script lang="ts">
import { defineComponent, type PropType } from 'vue'
import type { InputProps } from 'Components'

export default defineComponent({
  name: 'BaseInput',
  props: {
    type: {
      type: String as PropType<InputProps['type']>,
      default: 'text',
      required: true
    },
    name: {
      type: String as PropType<InputProps['name']>,
      required: true
    },
    value: {
      type: String as PropType<InputProps['value']>,
      default: '',
      required: true
    },
    error: {
      type: Boolean as PropType<InputProps['error']>,
      default: false,
      required: false
    },
    label: {
      type: String as PropType<InputProps['label']>,
      required: true
    },
    errorMessage: {
      type: String as PropType<InputProps['errorMessage']>,
      default: 'This field is required.'
    },
    placeholder: {
      type: String as PropType<InputProps['placeholder']>,
      default: '',
      required: false
    },
    required: {
      type: Boolean as PropType<InputProps['required']>,
      default: false
    },
    classnames: {
      type: String as PropType<InputProps['classnames']>,
      default: '',
      required: false
    },
    iconClasses: {
      type: String as PropType<InputProps['iconClasses']>,
      default: '',
      required: false
    },
    maxLength: {
      type: Number as PropType<InputProps['maxLength']>,
      default: undefined,
      required: false
    },
    min: {
      type: Number as PropType<InputProps['min']>,
      default: undefined,
      required: false
    },
    max: {
      type: Number as PropType<InputProps['max']>,
      default: undefined,
      required: false
    },
    formatinput: {
      type: Function as PropType<InputProps['formatinput']>,
      default: undefined,
      required: false
    },
    handleKeyPress: {
      type: Function as PropType<InputProps['handleKeyPress']>,
      default: undefined,
      required: false
    },
    validate: {
      type: Function as PropType<InputProps['validate']>,
      default: undefined,
      required: false
    }
  },
  emits: ['update:value'],
  data() {
    const showPassword: boolean = false
    return {
      showPassword,
      typeError: false,
      computedValue: this.value
    }
  },
  computed: {
    styles() {
      const emailPasswordTextStyle =
        'border border-black/10 rounded-sm lg:rounded-md p-2 text-xs block w-full appearance-none focus:outline-none focus:ring-0 relative lg:text-sm'
      const rtn: {
        [key: string]: string
      } = {
        email: emailPasswordTextStyle,
        password: emailPasswordTextStyle,
        text: emailPasswordTextStyle
      }

      return rtn
    }
  },
  methods: {
    handleInput(e: Event) {
      const target = e.target as HTMLInputElement
      // format input
      const value = this.formatinput ? this.formatinput(target.value) : target.value
      // update value
      this.$emit('update:value', value)
      // validate
      this.typeError = this.validate ? this.validate(value) : false
      this.$forceUpdate()
    },
    togglePassword(e: Event) {
      e.preventDefault()
      this.showPassword = !this.showPassword
    }
  }
})
</script>
