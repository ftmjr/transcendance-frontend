<template>
  <a class="border-green-400/30 block cursor-pointer" @click="handleRead">
    <div v-if="isShort" :class="['flex justify-between items-start gap-1 py-3 px-3 rounded-md gap-4', notification.status === 'READ' ? 'bg-[#343851]/30' : 'bg-green-600/30']"
  >
...
const handleRead = (e: Event) => {
  ...
  router.push(`/users/show/${notification.referenceId}`);
}
</script>
