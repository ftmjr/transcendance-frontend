        ...
        <v-card>
          ...
          <v-card-text>
            ...
            <v-text-field
                ...
                type="password"
            />
            <v-switch v-model="chatStore.createInfo.protected" label="Protected" color="indigo"></v-switch>
            <v-switch v-model="chatStore.createInfo.private" label="Private" color="indigo"></v-switch>
          </v-card-text>
          <v-card-actions>
            <v-btn :color="isCreatable ? 'primary' : 'disabled'" @click="chatStore.createRoom()" :disabled="!isCreatable">
              Create
            </v-btn>
...
export default defineComponent({
  ...
  computed: {
    isCreatable() {
      if (this.chatStore.createInfo.name === '') { return false }
